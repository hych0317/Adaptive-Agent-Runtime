...
- [x] 构造超过上下文预算的大量客服记录。
- [x] 保存最终模型投影并证明金额约束不在其中。
- [x] Stub 提出超额退款，领域权威校验拒绝。
- [x] 相同上下文压力下合法退款正向对照成功。

### M1

- [x] tenant scope 跨租户隔离。
- [x] 同租户 user Principal 条件隔离。
- [x] 分别检查候选、Recall Bundle 和模型上下文。
- [x] U1 自己的后续运行能够合法召回。

### M2

- [x] 全局快速配送与礼物隐藏价格条件共存。
- [x] 全局快速配送与单订单标准配送例外共存。
- [ ] 可选实现全局 MODIFY，并验证新 revision 和历史保留。

### 敏感信息

- [x] 地址、支付 token 和凭据候选写入被确定性拒绝。
- [x] canary 扫描覆盖 Memory、模型上下文、Audit 和报告。
...
- [x] 复用现有 LLM Gateway 和配置方式，不在场景文件保存凭据。
- [x] 固定模型目标、版本、参数、预算和 Prompt/Schema 指纹。
- [x] 支持选择案例、重复次数、Profile 和输出目录。
- [x] 记录危险提案、拦截、安全恢复路径、合法完成和人工审批。
- [x] 汇总原始计数、比例和小样本区间。
- [x] 将模型服务失败与 AAR 机制失败分开统计。
- [x] 在模型输出规范化后、Runtime 治理前提供显式字段级故障注入，并覆盖 P1、P2 三种篡改、P4、P5、C1 的拒绝后恢复矩阵。
- [x] 支持从公开 TOML 目标与被忽略的私有 TOML 凭据运行有界真实模型实验，密钥不进入配置指纹或报告。
